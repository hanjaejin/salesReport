...
| 버전 | v1.3 (기획서 v1.1.1 정합화: 운영자 품질 화면 채택, 파일럿 데이터 범위 13개월, 3-Gate 판정 구조, Hard Gate-테스트 매핑, 보고 종료 지표 격상) |
| 작성일 | 2026-07-23 |
| 작성 | IT 담당 한재진 |
...
- **FR-5.4 배치 알림·운영자 품질 화면**: 실패·지연·미수신·대사오류를 운영자에게 알리고 마지막 성공 데이터는 계속 제공한다. **운영자 품질 화면을 1차 범위에 포함한다** — 배치 상태, 품질 대사 결과, 미분류·미설명 건수를 한 화면으로 제공한다. (당초 '알림만' 축소 채택했던 결정의 공식 변경: Hard Gate의 '대사 오류 0·중복 0'을 심사에서 증명하려면 게이트 결과를 보여주는 증거 화면이 필요해졌기 때문이다)
...
최초 이관은 과거 5년 상세를 월·일 단위로 분할 적재하여 상세 사실 5년과 집계 마트 5년을 함께 구축한다. 용량(원본 300~400GB `[추정]`, SSD 1TB)은 5.7의 용량시험으로 확정하며, 파일럿은 대상 점포 1~3곳 × **최근 13개월**을 기본 적재한다(전주 동요일·전년 동기 비교 근거 확보). 5년 전체 이관은 심사 MVP 성공조건과 분리하고, 이관 성능은 표본 구간 실측(`EV-PERF`)으로 검증해 정식 단계 이관 계획의 근거로 쓴다.
...
| **기존 보고 종료 (대표 지표)** | 자동 보고 승인 후 폐지된 양식·채널 수 — 승인 문서가 증거이므로 조작 불가능하며 조직 변화를 직접 측정한다 | 정식 파일럿 Go 조건: 최소 1개 양식 종료 승인 확보 |
...
| 9월 초 | 설계서 제출 시점 최신 확정본, 구현체, Evidence, 파일럿 결과 | 제출 패키지 완성 |
...
확산 판정은 기획서 4.3의 **3단계 Gate 구조**를 따른다. 판정 기준은 파일럿 시작 전에 고정하며 사후 변경을 금지한다.

1. **기술 Hard Gate (전제조건)**: 중복매출 0, 미검증 문장 노출 0, 금지컬럼 적재 0, 권한 우회 0, 실패 시 마지막 성공본 유지, 미설명 대사오류 0, 재현성, 복구시험 — 하나라도 실패하면 채택 성과와 무관하게 확산을 보류한다. 각 항목의 증명 방법은 11.5 매트릭스의 Hard Gate 열과 1:1로 연결된다.
2. **현장·업무 Adoption Gate**: 자발접속(3초 내 이탈 병기), 현장적합률, 오류신고율, 계속사용·추천 의향, 수작업 X분→자동확인 Y분 변화, **기존 보고 종료 승인 가능성**.
3. **재사용 Gate**: 두 번째 어댑터에서 공통 엔진 핵심 모듈 유지 여부와 변경량 기록.
...
각 시험은 어느 Hard Gate를 증명하는지 명시한다 — "0건"은 주장이 아니라 아래 시험의 실행 결과로만 성립한다.

| 영역 | 핵심 시험 | 통과 기준 예시 | 증명하는 Hard Gate |
|---|---|---|---|
| 단위 | 취소·반품·할인·객단가·증감률 | 사전 정의 케이스 100% | 미설명 대사오류 0 |
| 데이터 | 키 중복, ITEMCNT/TENDERCNT, 금액, 미분류 | 미설명 오류 0 | 중복매출 0 · 미설명 대사오류 0 |
| 통합 | Oracle 추출→STAGE→FACT→MART→화면 | 동일 기준값 재현 | 중복매출 0 |
| 회귀 | 거래규칙·프롬프트 변경 전후 | 승인하지 않은 지표변화 0 | 재현성 |
| AI | 숫자·대상·기간·방향·근거·금지원인 | 미검증 문장 표시 0 | 미검증 문장 노출 0 |
| 성능 | 1일·7일·1개월 적재, 첫 접속, 동시접속 | 목표 또는 병목 근거 확보 | (Adoption Gate 근거) |
| 장애 | Ollama 중단, 적재 중단, 파일 손상 | 템플릿 폴백·마지막 성공본·복구 | 실패 안전 · 복구 |
| 보안 | 타점포 접근, 금지컬럼, 외부호출 | 우회 접근·금지컬럼·CDN 0 | 권한 우회 0 · 금지컬럼 0 |
| 사용성 | 고연령 사용자 핵심 과업 | 성공률·시간·오클릭 기록 | (Adoption Gate 근거) |
| 재현성 | 동일 입력·규칙·버전 | 동일 계산결과·릴리스 식별 | 재현성 |
...
| 32 | Hard Gate "0건"은 주장이 아니라 시험 결과 — 게이트와 테스트의 1:1 매핑이 증명력을 만든다 | 11장 |
| 33 | "종료된 보고 양식 수"는 조작 불가능한 조직 변화 지표 — 승인 문서가 곧 증거 | 9장 |
| 34 | 품질 화면은 곧 증거 화면 — Hard Gate를 심사에서 보여주려면 게이트 결과의 시각화가 필요 | 4, 7장 |
