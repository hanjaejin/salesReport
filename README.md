...
스키마 생성부터 적재까지 이 한 줄로 끝난다. 옮기는 것은 **133,185행**(마트 3종 + 브리핑 + 여러 매장 요약 + 점포)이고,
...
실측(2026-08-27): 연결 확인 포함 **11.8초**. Supabase에서 다시 세어 6개 테이블 행수가
정확히 일치했고, 원격 DB만 보고 화면을 돌려 **세 매장 브리핑·추세·기간합계·보고서 4시트가
로컬과 완전히 동일**함을 확인했다 (원격 브리핑 조회 30~200ms).

> ℹ️ 화면의 **[최근 7일 재생성]** 버튼을 클라우드에서 누르면 원장 **32,790행**이
